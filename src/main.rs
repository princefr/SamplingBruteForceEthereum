...
#[path = "../implementations/address.rs"]
mod address;
...
fn generate_random_mnemonic() -> Mnemonic {
    ...
    let mnemonic = Mnemonic::random(&mut rng, Language::default());
    return mnemonic;
}

fn main() {
    println!("Loading {}...", "data".green());
    let balances = read_csv_file("./bq-results.csv");
    println!("CSV file loaded, found {} wallets", balances.len());
    let secp = Secp256k1::new();

    // Set the number of threads for Rayon
    let num_threads = 12; // Adjust as needed
    ThreadPoolBuilder::new()
        .num_threads(num_threads)
        .build_global()
        .unwrap();
    let start: u128 = 0;
    let end: u128 = 1000000;
    

    loop {
        // infintly itererate
        let mnemonics = (start..end)
            .into_par_iter()
            .map(|_| generate_random_mnemonic())
            .filter(|_| rand::random::<f64>() < 0.00001)
            .map(|m| m.to_seed("") );

        let xprvs = mnemonics
            .into_par_iter()
            .map(|seed| XPrv::new(&seed));

        let unwrapped = xprvs.into_par_iter().map(|xprv| xprv.unwrap());
        let xprvs_clone = unwrapped.clone();
        let start_time = std::time::Instant::now();

        unwrapped.into_par_iter().for_each(|xprv| {
            let private_key = xprv.private_key();
            let secret_key = SecretKey::from_slice(&private_key.to_bytes()).unwrap();
            let public_key = PublicKey::from_secret_key(&secp, &secret_key);
            let address = address::Address::extended_public_key_to_address(public_key);
            // Consume the function only if there is something inside.
            if balances.contains_key(&address) {
                println!("{}: {}", address.green(), secret_key.display_secret().to_string().green());
            }
        });
        ...
        let addresses_processed = xprvs_clone.count();
        let addresses_processed_per_second =
            (addresses_processed as f64) / (elapsed_time.as_secs_f64());
        println!("Addresses processed per second: {}  address_processed: {}", addresses_processed_per_second.to_string().yellow(), addresses_processed.to_string().yellow()
        );
    }
}

// Read the csv file and return a hashmap with the addresses
fn read_csv_file(file_url: &str) -> HashMap<String, i32> {
    let mut balances_map: HashMap<String, i32> = HashMap::new();
    let file = File::open(file_url).unwrap();
    let mut reader = Reader::from_reader(file);
    for result in reader.deserialize() {
        if result.is_ok() {
            let balance: Balances = result.unwrap();
            balances_map.insert(balance.address, 0);
        }
    }
    return balances_map;
}
...
fn generate_random_mnemonic_from_words(words: Vec<String>) -> String {
    let mut mnemonic = String::new();
    for i in 0..12 {
        let random_index = rand::random::<usize>() % words.len();
        mnemonic.push_str(&words[random_index]);
        if i != 11 {
            mnemonic.push(' ');
        }
    }
...
// calculate the number of apparitions of each word in the file
fn calculate_numbers_of_word_apparitions(file_url: &str) -> HashMap<String, usize> {
    let mut word_apparitions: HashMap<String, usize> = HashMap::new();
    let file = File::open(file_url).unwrap();
    let reader = BufReader::new(file);
    let wallets: Vec<Wallet> = serde_json::from_reader(reader).unwrap();
    for wallet in wallets {
        for word in wallet.mnemonic.split(" ") {
            let count = word_apparitions.entry(word.to_string()).or_insert(0);
            *count += 1;
        }
    }
...
/// .
// print the number of apparitions of each word in the file
fn print_words_with_apparitions(file_url: &str) {
    let word_apparitions = calculate_numbers_of_word_apparitions(file_url);
    for (word, apparitions) in word_apparitions {
        println!("{}: {}", word, apparitions);
    }
}

/// .
fn print_4_most_used_words(file_url: &str) {
    let word_apparitions = calculate_numbers_of_word_apparitions(file_url);
    let mut words: Vec<(&String, &usize)> = word_apparitions.iter().collect();
    words.sort_by(|a, b| b.1.cmp(a.1));
    for (word, apparitions) in words.iter().take(4) {
        println!("{}: {}", word, apparitions);
    }
}

/// .
fn print_4_less_used_words(file_url: &str) {
    let word_apparitions = calculate_numbers_of_word_apparitions(file_url);
    let mut words: Vec<(&String, &usize)> = word_apparitions.iter().collect();
    words.sort_by(|a, b| a.1.cmp(b.1));
    print!("less used word");
    for (word, apparitions) in words.iter().take(4) {
        println!("{}: {}", word, apparitions);
    }
}

/// . Select the 500 most used words in the files
fn select_500_words_most_used(file_url: &str) -> Vec<String> {
    let word_apparitions = calculate_numbers_of_word_apparitions(file_url);
    let mut words: Vec<(&String, &usize)> = word_apparitions.iter().collect();
    words.sort_by(|a, b| b.1.cmp(a.1));
    let mut selected_words: Vec<String> = Vec::new();
    for (word, _) in words.iter().take(512) {
